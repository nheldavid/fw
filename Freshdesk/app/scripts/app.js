...
async function updateTicketData() {
    try {
    const data = await appState.client.interface.trigger("setValue", {
            id: "cf_customer_name",
            value: "CX-006"
            });
        // let data = await appState.client.db.set("ticket:5", {
        // "description_text": "CX-2"
        // });
        ...
        console.log(data);
        //console.log(appState.client.data.get("ticket"));
    }
    // data: { message: "ok" }
    catch (error) {
    // error object { error: <error_msg> }
    console.log(error);
    }   
}
...
function HideFields() {
    hide('cf_customer_name');
    hide('cf_order_number');
    hide('cf_delivery_date');
    hide('cf_order_status');
}

const hide = async (element) => {
  try {
    await appState.client.interface.trigger("hide", { id: element });
  } catch (error) {
