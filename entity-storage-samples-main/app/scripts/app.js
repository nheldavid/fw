...
async function onAppActivate(client) {
  await debugEntityAccess(client);
...
function prepareEntity(client) {
  const entity = client.db.entity({ version: "v3" });
  return entity.get("todos");
}

async function createToDo(todos, name) {
  return await todos.create({ name, status: "ToDo" });
}

async function markAsDone(todos, todoId, name) {
  return await todos.update(todoId, { name, status: "Done" });
}
...
// Add this to test before your main app logic
async function debugEntityAccess(client) {
  try {
    const entity = client.db.entity({ version: "v1" });
    const todos = entity.get("todos");
    const test = await todos.getAll({ limit: 1 });
    console.log("✅ Entity access working:", test);
    return true;
  } catch (error) {
    console.error("❌ Entity access failed:", error);
    if (error.message.includes("entity not found")) {
      console.error("SOLUTION: Create 'todos' custom object in Freshdesk Admin");
    }
    return false;
  }
}